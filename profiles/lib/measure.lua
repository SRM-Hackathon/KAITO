...
local default_maxwidth = 4
local width_non_numerical_values = Set { "default", "none", "no-sign", "unsigned" }
function Measure.get_max_width(raw_value, element)
  if raw_value then
    if width_non_numerical_values[raw_value] then
      if element then
        return element:get_location_tag('maxwidth') or default_maxwidth
      else
